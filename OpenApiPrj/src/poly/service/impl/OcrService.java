...
public class OcrService implements IOcrService{
	...
	public OcrDTO getReadforImageText(OcrDTO pDTO) throws Exception {
		...
		// OCR 분석에 필요한 기분데이터(이미 각 나라의 언어별로 학습시킨 데이터 위치 폴더)
		// 저장 경로는 물리결로를 사용함 (전체경로)
...
//		instance.setLanguage("eng"); // 영어 설정
		...
		log.info(this.getClass().getName() + ".getOcrInfoFromWEB end!!");
		log.info("pDTO : " + pDTO);
		ocrMapper.saveImageText(pDTO);	
		log.info("pDTO : " + pDTO.getFileName());
		log.info("pDTO : " + pDTO.getFilePath());
