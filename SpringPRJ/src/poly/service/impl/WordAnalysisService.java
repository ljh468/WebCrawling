...
public class WordAnalysisService implements IWordAnalysisService{
	...
	// 생성자 사용함 - 톰캣에서 부팅할때 @service를 모두 메모리에 올림
	// 톰캣이 메모리에 올릴때, 생성자에 선언한 Komoran도 같이 메모리에 올라가도록 생성자에 코딩함
	...
	public WordAnalysisService() {
		...
		// NLP 분석 객체 메모리 로딩
		this.nlp = new Komoran(DEFAULT_MODEL.LIGHT); // 학습 데이터 경량화 버젼(웹서비스에 적합함)
//		this.nlp = new Komoran(DEFAULT_MODEL.FULL); // 학습 데이터 전체 버젼(일괄처리 : 배치 서비스에 적합함)
		
		// 데이터사전 로딩하기
		// 윈도우 운영체제의 파일은 C:/를 붙이지 않아도 메인(주)드라이브인 C를 인식함
		this.nlp.setUserDic("/myDic/wordDic.txt");
		log.info("난 톰캣이 부팅되면서 스프링 프레임워크가 자동실행되었고, 스프링 실행될 때 nlp 변수에 Komoran 객체를 생성하여 저장함");
	...
	public List<String> doWordNouns(String text) throws Exception {
		...
		// 분석할 문장의 앞 뒤에 존자할 수 있는 필요없는 공백제거
		String trim_text = replace_text.trim();
		
		log.info("분석할 문장의 앞 뒤에 존자할 수 있는 필요없는 공백제거" + trim_text);
	...
	public Map<String, Integer> doWordAnalysis(String text) throws Exception {
		...
		String newContext =// "아이오아이는 최고의 아이돌이며, 여자연예인 중에 멋진 걸그룹이다.";
				newsCollectService.doNaverNewsContents(
